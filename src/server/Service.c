...
(FILE * fp, char * dbIP, char * dbUserName, char * dbPass, char * dbName, int dbPort, int sock, char * buffer, char * data)
{
	...
	sprintf(sql, "%s%d%s%s%s%d%s%d", "update usr_info set usr_status=", USER_STATUS_ONLINE, 
	...
	for (i = 0; i < friendNum; i++)
	{
		...
		index += length;
		// get friend status
		data[index - data] = atoi(row[2]);
		index ++;
	}
	mysql_free_result(res);
	// query group list of this user
	sprintf(sql, "%s%d", "select i.group_id,group_name,group_online_member_num,group_member_num from group_info i,group_member_info m where i.group_id=m.group_id and m.member_id=", usrID);
	sql[152 + ((int)buffer[1])] = '\0';
	res = mysqlQueryGetRes(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql);	
	if ( ! res )
	{
...
(FILE * fp, char * dbIP, char * dbUserName, char * dbPass, char * dbName, int dbPort, int sock, char * buffer, char * data)
{
	...
	if ('1' == status)
	{
		...
		row = mysql_fetch_row(res);
		mysql_free_result(res);
		memcpy(friendIP, row[0], strlen(row[0]));
		...
		{
			sprintf(sql, "%s%d,%d,'%s')", "insert into usr_resend_info(usr_id,usr_friend_id,usr_resend_msg) values (", usrID, friendID, data);
			sql[78 + ((int)buffer[1]) + ((int)buffer[2]) + strlen(data)] = '\0';
			if (0 != mysqlInsUpdDel(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql))
			{
				sendFlagMsg(sock, SVR_MYSQL_UNREACHABLE);
...
(FILE * fp, char * dbIP, char * dbUserName, char * dbPass, char * dbName, int dbPort, int sock, char * buffer, char * data)
{
	...
	// put adder self into group_member_info table
	sprintf(sql, "%s%d,%d)", "insert into group_member_info(group_id,member_id) values(", groupID, usrID);
	sql[59 + groupIDLen +  length1] = '\0';
	if (0 != mysqlInsUpdDel(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql))
	{
		sendFlagMsg(sock, SVR_MYSQL_UNREACHABLE);
		ERROR_LOG(fp, tv, tim, "addGroup, mysql is unreachable when put adder self into group_member_info table\n");
		return 1;
	}
	...
	for (i=0; i<memNum; i++)
	{
		...
		temp[length4] = '\0';
		memID = atoi(temp);
		// put member info into group_member_info table
		sprintf(sql, "%s%d,%d)", "insert into group_member_info(group_id,member_id) values(", groupID, memID);
		sql[60 + groupIDLen +  length4] = '\0';
		if (0 != mysqlInsUpdDel(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql))
		{
			sendFlagMsg(sock, SVR_MYSQL_UNREACHABLE);
			ERROR_LOG(fp, tv, tim, "addGroup, mysql is unreachable when insert into group_member_info table\n");
			return 1;
		}
	...
	sprintf(sql, "%s%d", "select count(1) from usr_info i,group_member_info m where i.usr_id=m.member_id and i.usr_status=1 and m.group_id=", groupID);
	sql[113 + groupIDLen] = '\0';
	if (0 != mysqlQuerySimple(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql, temp))
	{
		sendFlagMsg(sock, SVR_MYSQL_UNREACHABLE);
...
(FILE * fp, char * dbIP, char * dbUserName, char * dbPass, char * dbName, int dbPort, int sock, char * buffer, char * data)
{
	...
	int rowsNum;
	int i;	
	int length1, length2, length3, friendIDLen;
	...
	index += length2;
	data[index - data] = buffer[1];
	index ++;
	memcpy(index, buffer + 2, length1);
	index += length1;
	length3 = (int)buffer[(3 + length1 + length2)];
	...
	sprintf(sql, "%s%d%s%d", "select member_id,usr_local_ip,usr_local_port from group_member_info m,usr_info i where m.member_id=i.usr_id and i.usr_status=1 and m.group_id=", groupID, " and not m.member_id=", usrID);
	sql[163 + length1 + length2] = '\0';
	res = mysqlQueryGetRes(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql);
	if ( ! res )
	{
		// all members of this group are offline
		sprintf(sql, "%s%d%s%d", "select member_id from group_member_info m where m.group_id=", groupID, " and not m.member_id=", usrID);
		sql[80 + length1 + length2] = '\0';
		res = mysqlQueryGetRes(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql);
		if ( ! res )
		{
		...
		for (i=0; i<rowsNum; i++)
		{
			...
			sprintf(sql, "%s%d,%d,'%s')", "insert into usr_resend_info(usr_id,usr_friend_id,usr_resend_msg) values (", usrID, friendID, data);
			sql[78 + length1+ friendIDLen + strlen(data)] = '\0';
			if (0 != mysqlInsUpdDel(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql))
			{
				sendFlagMsg(sock, SVR_MYSQL_UNREACHABLE);
	...
	sprintf(sql, "%s%d%s%d", "select member_id from group_member_info m,usr_info i where m.member_id=i.usr_id and i.usr_status=0 and m.group_id=", groupID, " and not m.member_id=", usrID);
	sql[163 + length1 + length2] = '\0';
	res = mysqlQueryGetRes(fp, dbIP, dbUserName, dbPass, dbName, dbPort, sql);
	if ( ! res )
	{

		...
		for (i=0; i<nfds; i++)
		{
			if(events[i].data.fd == sfd)
			{
				...
				switch (buffer[0])
				{
					...
					case 0x07:
					{
						logoutHandler(fp_log,argv[1],MYSQL_USRNAME,MYSQL_USRPASS,MYSQL_DBNAME,MYSQL_DBPORT,cfd,buffer,data);
						break;
					}
					case 0x08:
					{
						queryGroupMemberListHandler(fp_log,argv[1],MYSQL_USRNAME,MYSQL_USRPASS,MYSQL_DBNAME,MYSQL_DBPORT,cfd,buffer,data);
						break;
					}
					default:
					{
